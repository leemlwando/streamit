...
<div id="main">
    ...
    <section>
        <article>
            <pre class="prettyprint source linenums"><code>const {upload_prefix} = require("./config");
...
function cacheResults(result, {type, resource_type}) {
  if (result.responsive_breakpoints) {
    result.responsive_breakpoints.forEach(
      (options) => Cache.set(
        result.public_id,
        {type, resource_type, raw_transformation: options.transformation},
        options.breakpoints.map(i => i.width)
      ));
  }
}
...
function call_api(action, callback=utils.NOP, options, get_params) {
